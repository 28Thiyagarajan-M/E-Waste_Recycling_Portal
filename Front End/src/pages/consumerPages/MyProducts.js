...
export default function MyProducts() {
  ...
  useEffect(() => {
    axios
      .get(
        `http://localhost:9090/consumer/products/myProducts/${localStorage.getItem(
          "UserId"
        )}`
      )
      .then((response) => {
  ...
  const handleDelete = (id) => {
    try {
      axios
        .delete(`http://localhost:9090/consumer/products/deleteProduct/${id}`)
        .then((response) => {
      ...
      <div className="p-3">
        <Container className="text-center">
          ...
          <Button
            ...
            onClick={() => navigate("/consumer/addProduct")}
          >
            Add Product
          </Button>
          <Table
                  ...
                  <tr key={data.productId} className="p-2">
                    <td className="d-flex text-start align-middle">
                      ...
                      <Col className="px-2">
                        ...
                        <h6>{data.category}</h6>
                        <h6>Manufactured Year: {data.manufacturedYear}</h6>
                    ...
                    <td className="align-middle">
                      <ButtonToolbar className="justify-content-evenly">
                        <ButtonGroup className="me-2">
                          <Link
                            to={`/consumer/editProduct/${data.productId}`}
                            state={{ product: data }}
                          >
