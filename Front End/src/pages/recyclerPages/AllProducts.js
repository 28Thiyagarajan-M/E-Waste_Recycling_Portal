...
import { Table, Container, Image, Col, Button } from "react-bootstrap";
...
export default function AllProducts() {
  ...
  useEffect(() => {
    try {
      axios
        .get(`http://localhost:9090/recycler/products/notPickedUpProducts`)
        .then((response) => {
  ...
  const handlePickUp = (data) => {
    ...
    try {
      axios
        .put(
          `http://localhost:9090/products/pickUpProduct/${data.productId}`,
          productDto
        )
        .then((response) => {
